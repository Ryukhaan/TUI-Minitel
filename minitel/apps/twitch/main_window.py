...
class MenuDesktopWindow(Window):
    ...
    def draw_type_item(self, item, row: int = 0):
        if item == PREV_PAGE_LABEL:
            ...
            type_mixels = draw_text(self.rect.x, self.rect.y + row, "D", state=GraphicState(invert=True))
        type_mixels += draw_text(self.rect.x + 1, self.rect.y + row, "|")
        return type_mixels
    ...
    def draw_item(self, item: Path, row: int = 0) -> list[Mixel]:
        ...
        file_mixels = draw_text(self.rect.x + len(type_m), 
                self.rect.y + row,
                text,
                state=GraphicState(fg=text_color, invert=self.index == row))
